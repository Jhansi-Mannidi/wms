  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Total SKUs", value: "2,458", icon: <Package className="w-5 h-5 text-brand" />, sub: "+12 this week" },
          { label: "Stock Value", value: "₹51.2 Cr", icon: <BarChart2 className="w-5 h-5 text-brand" />, sub: "at cost" },
          { label: "Avg Turnover", value: "8.3x", icon: <TrendingUp className="w-5 h-5 text-success" />, sub: "annualised" },
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/50">
            <tr>{["Category","SKUs","Qty on Hand","Value","Turnover","Fill Rate"].map(h => <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">{h}</th>)}</tr>
          </thead>
