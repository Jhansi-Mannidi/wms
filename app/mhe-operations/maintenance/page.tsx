    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-card border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-muted/50"><tr>{["Task ID","Equipment","Type","Description","Assignee","Due Date","Status","Actions"].map(h=><th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide">{h}</th>)}</tr></thead>
              ...
              <tr key={t.id} className="hover:bg-muted/30 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1">
                    ...
                    {t.status !== "Completed" && (
                      <button onClick={() => advance(t)} title="Advance status" className="p-1.5 rounded-md text-success hover:bg-success/10 transition-colors">
